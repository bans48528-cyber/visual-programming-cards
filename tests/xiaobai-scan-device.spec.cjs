...
(async()=>{
  ...
  const pid=run('shell','pidof','com.xiaobai.remote').trim();
  ...
  run('shell','am','start','-W','-n','com.xiaobai.remote/com.cardprogramming.app.MainActivity');
  await page.waitForFunction(async()=> !(await Capacitor.Plugins.SparkBle.getState()).scanning);
  ...
  fs.writeFileSync(path.join(out,'xiaobai-scan-device-report.json'),JSON.stringify(report,null,2)+'\n');
  await page.screenshot({path:path.join(out,'xiaobai-bluetooth-phone.png')});
