...
(async () => {
  ...
  try {
    ...
    // Back cancels a draft first and keeps the parameter controls open.
    await back();
    assert.equal(await page.evaluate(() => program[0].params.count), 1);
    assert.equal(await page.locator('#paramEditor').isVisible(), true);
    await page.locator('#paramEditor .param-current').tap();
    await page.locator('.number-key[data-key="4"]').tap();
    await page.locator('.number-key[data-key="confirm"]').tap();
    await page.locator('#paramEditor .param-current').tap();
    ...
    assert.equal(new URL(page.url()).hash, '#editor');
    await page.locator('#bluetoothBtn').tap();
    await page.locator('.bluetooth-page').waitFor({state:'visible'});
    assert.match(await page.locator('.bt-stage-note').textContent(), /程序发送/);
    ...
    assert.equal(await page.evaluate(() => getProgramSnapshot()), snapshot);
    await page.locator('#tab-light').tap();
    await page.locator('#palette [data-card-id=matrix-display]').tap();
    await page.locator('#chain .matrix-param-trigger').tap();
    const cell = await page.locator('.matrix-cell').first().boundingBox();
    const previous = await page.evaluate(() => program[1].params.pattern[0]);
    await cdp.send('Input.dispatchTouchEvent', {type:'touchStart', touchPoints:[{x:cell.x+cell.width/2,y:cell.y+cell.height/2}]});
    await page.evaluate(() => window.nativeListeners.pause());
    await cdp.send('Input.dispatchTouchEvent', {type:'touchEnd', touchPoints:[]});
    assert.equal(await page.evaluate(() => matrixPaintState), null);
    await page.reload();
    assert.equal(await page.evaluate(() => program[1].params.pattern[0]), previous ? 0 : 1);
